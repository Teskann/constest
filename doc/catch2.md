# ConsTest with Catch2

## Configuration

To use ConsTest with Catch2, you need to `#define` the `CONSTEST_CONFIG_CATCH2` macro.

CMakeLists.txt:

```cmake
target_compile_definitions(your_target PRIVATE CONSTEST_CONFIG_CATCH2)
```
...
[See a more complete example](../tests/catch2/test_catch2.cpp)

## `CONSTEXPR_SECTION` macro

At runtime, the `CONSTEXPR_SECTION` will be transformed into a `SECTION` with the same description prefixed
by `[Passed at compile-time] `. The whole content of the `CONSTEXPR_SECTION` will be executed at runtime as well.

## Assertion macros

| ConsTest Macro            | Catch2 equivalent | Runtime call    |
|---------------------------|-------------------|-----------------|
...
> be pointless.
>
> Catch2's standard macros (`REQUIRE`, `CHECK`, etc.) should not be used inside a `CONSTEXPR_SECTION` because they
...
At runtime, the `CONSTEXPR_SECTION_DEBUG` will be transformed into a `SECTION` with the same description
prefixed
by `[WARNING - Debugging a CONSTEXPR_SECTION] `.

## Supported Catch2 versions

Any version of Catch2 supporting the macros above should work.
